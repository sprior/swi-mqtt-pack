...
static atom_t ATOM_clean_session;
...
typedef struct {
  ...
  bool          clean_session;          /* if true then connect with clean_session = true  */
  int           refs;
...
static int set_engine_for_callbacks(swi_mqtt *m) {
  ...
  if (m->is_async) {
    ...
    if (!current_engine) {
      ...
      _LOG("--- (f-x) called PL_create_engine\n");
    }
...
acquire_mqtt_symbol(atom_t symbol) { 
  ...
  m->symbol = symbol;
  m->refs++;
  _LOG("--- (f-b) acquire_mqtt_symbol swi_mqtt: %d new refs: %d\n", (int) symbol, m->refs);
}
...
release_mqtt_symbol(atom_t symbol) { 
  ...
  swi_mqtt *m = PL_blob_data(symbol, NULL, NULL);
  m->refs--;
  _LOG("--- (f-b) release_mqtt_symbol swi_mqtt: %d new refs: %d\n", (int) symbol, m->refs);
  destroy_mqtt(m);
  return TRUE;
}
...
c_mqtt_pub(term_t conn, term_t topic, term_t payload, term_t options) {
  ...
  if ( options ) { 
      ...
      while(PL_get_list(tail, head, tail)) { 
        ...
        if ( PL_get_name_arity(head, &name, &arity) && arity == 1 ) { 
            ...
            if ( name == ATOM_retain) { 
               if (!PL_get_bool(   arg, &retain)) {
                  result = FALSE;
                  _LOG("--- (f-c) c_mqtt_pub > get ATOM_retain failed\n");
                  goto CLEANUP;
               }
            } else if ( name == ATOM_qos )   {
               if (!PL_get_integer(arg, &qos)   ) {
                  result = FALSE;
                  _LOG("--- (f-c) c_mqtt_pub > get ATOM_qos failed\n");
                  goto CLEANUP;
               }
            }
  ...
  if (mosq_rc == MOSQ_ERR_SUCCESS) {
    _LOG("--- (f-c) c_mqtt_pub > publish done topic: %s\n", mqtt_topic);
    goto CLEANUP;
  } else {
    result = FALSE;
    _LOG("--- (f-c) c_mqtt_pub > publish failed rc: %d topic: %s\n", mosq_rc, mqtt_topic);
  }
...
c_mqtt_connect(term_t conn, term_t host, term_t port, term_t options) {
  ...
  if ( options ) { 
	...
	atom_t name;
        ...
        if ( PL_get_name_arity(head, &name, &arity) && arity == 1 ) { 
  	   ...
  	   if        ( name == ATOM_client_id )    {
              if (!PL_get_chars(  arg, &client_id,          CVT_WRITE | BUF_MALLOC) ) {
                 result = FALSE;
                 goto CLEANUP;
              }
            } else if ( name == ATOM_alias )        { 
               if (!PL_get_chars(  arg, &alias,              CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_module )       {
               if (!PL_get_chars(  arg, &hook_module,        CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_log )       {
               if (!PL_get_chars(  arg, &hook_on_log,        CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_connect)    {
               if (!PL_get_chars(  arg, &hook_on_connect,    CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_disconnect) {
               if (!PL_get_chars(  arg, &hook_on_disconnect, CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_message )   {
               if (!PL_get_chars(  arg, &hook_on_message,    CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_publish )   {
               if (!PL_get_chars(  arg, &hook_on_publish,    CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_subscribe)  {
               if (!PL_get_chars(  arg, &hook_on_subscribe,  CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_on_unsubscribe){
               if (!PL_get_chars(  arg, &hook_on_unsubscribe,CVT_WRITE | BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_hooks )        {
               if (!PL_get_bool(   arg, &use_hooks)       ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_debug_hooks)   {
               if (!PL_get_bool(   arg, &use_debug_hooks) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_is_async )     {
               if (!PL_get_bool(   arg, &is_async)        ) {
                  result = FALSE;
                  goto CLEANUP;
                }
            } else if ( name == ATOM_clean_session )     {
               if (!PL_get_bool(   arg, &clean_session)        ) {
                  result = FALSE;
                  goto CLEANUP;
                }
  	    } else if ( name == ATOM_keepalive )    {
               if (!PL_get_integer(arg, &keepalive)       ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            }
  ...
  m->is_async = is_async;
  m->clean_session = clean_session;
  m->is_async_loop_started = false;
  m->keepalive = keepalive;

  // pass swi_mqtt as user object (will be available in all callbacks)
  if (clean_session){
    _LOG("--- (f-c) c_mqtt_connect > clean_session: true\n");
  } else {
    _LOG("--- (f-c) c_mqtt_connect > clean_session: false\n");
  }
  mosq = mosquitto_new(client_id, clean_session, m);
  if (mosq) {
    m->is_in_use = TRUE;
...
static foreign_t 
c_mqtt_loop_forever(term_t conn) {
  swi_mqtt *m;
  
  _LOG("--- (f-c) c_mqtt_loop_forever\n");

  if (!get_swi_mqtt(conn, &m)) { return FALSE; }

  if (mosquitto_loop_forever(m->mosq, 10, 1) == MOSQ_ERR_SUCCESS) {
    return TRUE;
  }
  return FALSE;
}
...
c_mqtt_sub(term_t conn, term_t topic, term_t options) {
  ...
  if (!get_swi_mqtt(conn, &m)) {
    _LOG("--- (f-c) ERROR: c_mqtt_sub could not get connection\n");
    result = FALSE;
    goto CLEANUP;
  }
  ...
  if (!PL_get_chars(topic, &mqtt_topic, CVT_WRITE | BUF_MALLOC)) { 
    _LOG("--- (f-c) ERROR: c_mqtt_sub could not get topic\n");
    result = FALSE;
    goto CLEANUP;
  }
/*
  if ( options ) { 
      ...
      while(PL_get_list(tail, head, tail)) { 
        ...
        if ( PL_get_name_arity(head, &name, &arity) && arity == 1 ) { 
            term_t arg = PL_new_term_ref();
            _PL_get_arg(1, head, arg);
            if ( name == ATOM_payload_type ) {
               if (!PL_get_chars(  arg, &payload_type, CVT_WRITE|BUF_MALLOC) ) {
                  result = FALSE;
                  goto CLEANUP;
               }
            } else if ( name == ATOM_qos          ) {
...
*/
  ...
  if (mosq_rc == MOSQ_ERR_SUCCESS) {
    _LOG("--- (f-c) c_mqtt_sub > subscribe done topic: %s\n", mqtt_topic);
    result = TRUE;
  } else {
    _LOG("--- (f-c) c_mqtt_sub > subscribe failed rc: %d topic: %s\n", mosq_rc, mqtt_topic);
  }
...
c_mqtt_unsub(term_t conn, term_t topic) {
  ...
  mosq_rc = mosquitto_unsubscribe(m->mosq, NULL, mqtt_topic);
  if (mosq_rc == MOSQ_ERR_SUCCESS) {
    result = TRUE;
...
install_mqtt(void) {
  ...
  ATOM_clean_session  = PL_new_atom("clean_session");
  ...
  PL_register_foreign("c_mqtt_loop_forever",  1, c_mqtt_loop_forever,  0);
